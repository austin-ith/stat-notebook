...
```{r}
glm1 <- glm(birthyear == 88 ~ birthmonth, data = KidsFeet, family = binomial)
b <- glm1$coefficients
summary(glm1)
```
...
```{r}
glm2 <- glm(birthyear == 88 ~ width, data = KidsFeet, family = binomial)
...
```{r}
x <- predict(glm1, data.frame(birthmonth = 5), type = "response")
x
```
...
```{r}
pairs(RT, panel = panel.smooth, pch=16, col=rgb(.2,.2,.2,.1))
pairs(RT, panel = panel.smooth, pch=16)
```
...
```{r}
glm1 <- glm(precipOccurred ~ lowtemp, data = RT, family = binomial)
summary(glm1)

glm2 <- glm(precipOccurred ~ cloudcover, data = RT, family = binomial)
summary(glm2)

glm3 <- glm(precipOccurred ~ fall, data = RT, family = binomial)
summary(glm3)
```
...
```{r}
glm1 <- glm(precipOccurred ~ cloudcover + lowtemp, data = RT, family = binomial)
summary(glm1)

glm2 <- glm(precipOccurred ~ cloudcover + spring, data = RT, family = binomial)
summary(glm2)

glm3 <- glm(precipOccurred ~ cloudcover + fall, data = RT, family = binomial)
summary(glm3)

glm4 <- glm(precipOccurred ~ cloudcover + summer, data = RT, family = binomial)
summary(glm4)

glm5 <- glm(precipOccurred ~ cloudcover + weekday, data = RT, family = binomial)
summary(glm5)
```
