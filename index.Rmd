...
Here are some example of graphics to use for different data sets. 
...
Typical Questions for this data:

* One Sample t-Test
  + Does, on average, a certain soporific drug increased the hours of sleep of individuals?
  
* Paired Sample t-Test
  + Is there a difference between the reaction time of Men and Women?
  + Is there a difference in hours of extra sleep each individual received under the two drugs?
  + The length of a child’s foot is the same as 3x the width of their foot?
  
* Wilcoxen Signed Rank Test
  + Is there a difference in the median of hours of extra sleep each individual received under the two drugs?
  + Is there a difference in the center of the distributions of the heights?

When using a T-Test, a boxplot and qqplot are very helpful. 
...
Use a Histogram when comparing the frequency of Quantative Variables. Use a bar chart when comparing the frequency of Qualitative Variables
...
* [Paired Sample t-Test](tTests.html)
  ...
  + Examples: 
    + (Mine)[High School Seniors](./Analyses/t Tests/HighSchoolSeniors.html)
    + [sleepPaired](./Analyses/t Tests/Examples/SleepPairedt.html)
    + [StudentPaired](./Analyses/t Tests/Examples/Student1Paired.html)


* [Wilcoxen Signed Rank Test](WilcoxonTests.html)
  + $H_0: \text{median of differences} = 0$
  + $H_a: \text{median of differences} \neq 0$
  + Examples: 
    + [sleep](./Analyses/Wilcoxon Tests/Examples/SleepPairedWilcoxon.html)
    + [CornHeights](./Analyses/Wilcoxon Tests/Examples/CornHeightsPairedWilcoxon.html)
...
Typical Questions for this type of data:

* Independent Sample t-Test (Parametic)
  + Is the difference in average hours of extra sleep for each drug different from zero?
  + Is the difference between the volume of users in Spring and Summer is different than zero?
  + Are more users on the trail during days with no precipitation?
  
* Wilcoxen Rank Sum Test (Non-parametric)
  + Which bug spray is more effective at killing flies (bugs), A or B? Is the difference in medians equal to zero?
  + Is there a difference in mobility score between eastern cities and western cities? Is the difference in medians equal to zero?
  

When using a Independent T-Test or a Wilcoxon Ranks Sum Test, Boxplots become really useful when comparing summary data of two separate groups. 
...
* [Independent Sample T-Test](tTests.html)
  + Examples: 
    + [sleepInd](./Analyses/t Tests/Examples/SleepIndependentt.html)
    + [student1](./Analyses/t Tests/Examples/student1Independent.html)
    + [student2](./Analyses/t Tests/Examples/student2Independent.html)


* [Wilcoxen Rank Sum (Mann-Whitney)Test](WilcoxonTests.html)
  + Examples: 
    + [BugSpray](./Analyses/Wilcoxon Tests/Examples/BugSprayWilcoxonRankSum.html)
    + [MoralIntegration](./Analyses/Wilcoxon Tests/Examples/MoralIntegration.html)
...
* One-Way ANOVA
  + Is the average chick weight the same for each feed type? Or is at least one feed type different on average than the others?

* Two-Way ANOVA
  + Does the type of wool affect the avereage number of breaks? Does the tension affect the average number of breaks? Is there an interaction between wool and tension?
  
* Kruskal-Wallis Rank Sum Test (Non-Parametric)
  + How much do BYU-Idaho students make hourly during their off-track? Do they earn more as they gain more education?

Use Fitted Values Plots, QQ-Plots, and xy Plots as shown in DayCare example:

[DayCare](./Analyses/ANOVA/DayCare.html)
...
* [One-Way ANOVA](ANOVA.html)
  + Examples: 
    + [chickwts](./Analyses/ANOVA/Examples/chickwtsOneWayANOVA.html)

* [Two-Way ANOVA](ANOVA.html)
  + Examples: 
    + [Warpbreaks](./Analyses/ANOVA/Examples/warpbreaksTwoWayANOVA.html)
    + (MINE)[DayCare](./Analyses/ANOVA/DayCare.html)

* [Kruskal-Wallis Rank Sum Test](Kruskal.html)
  + Examples: 
    + [wagees](./Analyses/Kruskal-Wallis Test/Examples/wagesKruskal.html)
    + [pigweights](./Analyses/Kruskal-Wallis Test/pigweightsKruskal.html)
